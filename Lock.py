...
def cPrint(text, Return=False):
    printText = text.replace('cWhite', '[37m').replace('cGreen', '[92m').replace('cRed', '[91m').replace('cBlue', '[94m')
    if Return:
        return printText
    print(printText)
...
def logo():
    os.system('clear')
    print('[94m┌────────────────────────────────────────┐'.center(columns + 4))
    print('[94m│           [92m▌  ▗    ▌       ▌ ▌[94m          │'.center(columns + 14))
    print('[94m│           [92m▌  ▄ ▛▀▖▌▗▘ ▄▄▖ ▝▞ [94m          │'.center(columns + 14))
    print('[94m│           [92m▌  ▐ ▌ ▌▛▚      ▞▝▖[94m          │'.center(columns + 14))
    print('[94m│           [92m▀▀▘▀▘▘ ▘▘ ▘     ▘ ▘[94m          │'.center(columns + 14))
    print('[94m│                              [94m          │'.center(columns + 8))
    print('[94m│ [95mAuthor : Mr.Thonxyzz404                [94m│'.center(columns + 14))
    print('│ [95mTool   : Hack With Link                [94m│'.center(columns + 8))
    print('[94m└────────────────────────────────────────┘'.center(columns + 4))
...
def update():
    ...
    if not oldVersion == newVersion:
        ...
        os.system('cd ../ && rm -rf Link-x && git clone --depth 32 https://github.com/QrwszXnXnchommed0e56/Pshing > /dev/null 2>&1')
        psb('\n    [cGreen*cWhite] Update Complete!')
        psb('    [cGreen*cWhite] Starting new Version...')
        os.system('cd ../Link-x && python Lock.py')
        return
...
def printDetails(tmp):
    ...
    timeZone = 'None'
    ISP = 'None'
    if 'error' not in ipDetails and (not ipDetails == ''):
        try:
...
def getTunnel():
    ...
    if op == '4':
        sys.exit('')
    if op == '2' and auth in addedToken and (int(trialLeft) < 1):
        time.sleep(0.5)
...
def menu():
    ...
    if op == '00' or op == '0':
        sys.exit('')
    while op not in ['0', '1', '2', '3', '4', '5', '6', '7', '8']:
